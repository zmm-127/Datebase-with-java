...
public class ClientProcess implements Runnable{		//实现Runnable接口
	...
	private ClientManage cm;//新建ClientManage对象
	public ClientProcess(Socket socket,ClientManage cm) {//初始化对象
		this.socket = socket;
		this.cm=cm;
	}
	public void run() {
		try {
			...
			while(true) {//客户端一直保留登录状态，读取客户端内容，直到客户端关闭或输入logout
				if(reader==null||writer==null) {//若读写数据均为空则退出程序
					return;
				}
